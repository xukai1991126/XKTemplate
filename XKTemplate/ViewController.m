...
- (void)viewDidLoad {
    ...
    // 获取网络缓存大小
    PPLog(@"网络缓存大小cache = %fKB",[NetworkCache getAllHttpCacheSize]/1024.f);
...
{
    // 登陆
    [HTTPRequest getLoginWithParameters:@"参数" success:^(id response) {
    ...
    // 退出
    [HTTPRequest getExitWithParameters:@"参数" success:^(id response) {
...
- (void)getData:(BOOL)isOn url:(NSString *)url
{
    ...
    if(isOn)
    {
        ...
        [NetworkHelper GET:url parameters:para responseCache:^(id responseCache) {
            // 1.先加载缓存数据
    ...
    else
    {
        ...
        [NetworkHelper GET:url parameters:para success:^(id responseObject) {
            self.networkData.text = [self jsonToString:responseObject];
        } failure:^(NSError *error) {
...
- (void)monitorNetworkStatus
{
    ...
    [NetworkHelper networkStatusWithBlock:^(PPNetworkStatusType networkStatus) {
...
- (IBAction)download:(UIButton *)sender {
    ...
    if(!self.isDownload)
    {
        ...
        task = [NetworkHelper downloadWithURL:downloadUrl fileDir:@"Download" progress:^(NSProgress *progress) {
